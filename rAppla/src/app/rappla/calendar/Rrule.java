 ...
 * URL for more info: <a
 * href="http://code.google.com/p/google-rfc-2445/">http://
 * code.google.com/p/google-rfc-2445/</a>
 * 
 * @version $Id: Rrule.java,v 1.8 2007/05/31 16:33:30 cknudsen Exp $
...
public class Rrule extends Property implements Constants
{
	 ...
	 */
	public Rrule(String icalStr) throws ParseException, BogusDataException
	{
		super(icalStr);
		...
		for (int i = 0; i < args.length; i++)
		{
			...
			if (param.length != 2)
				throw new ParseException("Invalid RRULE '" + args[i] + "'",
						icalStr);
			String aname = param[0].toUpperCase(Locale.ENGLISH);
			String aval = param[1].toUpperCase(Locale.ENGLISH);
			...
			} else if (aname.equals("BYSETPOS"))
			{
				...
				for (int j = 0; j < s.length; j++)
				{
					if (StringUtils.isNumber(s[j], true))
					{
						...
						{
							throw new BogusDataException(
									"Invalid RRULE BYSETPOS (range) '"
											+ bysetpos[j] + "'", icalStr);
						}
					} else
					{
						throw new BogusDataException("Invalid RRULE BYSETPOS '"
								+ s[j] + "'", icalStr);
					}
				}
			...
			} else if (aname.equals("BYMONTHDAY"))
			{
				...
				for (int j = 0; j < s.length; j++)
				{
					if (StringUtils.isNumber(s[j], true))
					{
						...
						{
							throw new BogusDataException(
									"Invalid RRULE BYMONTHDAY (range) '"
											+ bymonthday[j] + "'", icalStr);
						}
					} else
					{
						throw new BogusDataException(
								"Invalid RRULE BYMONTHDAY '" + s[j] + "'",
								icalStr);
					}
				}
	 ...
	 * URL for more info: <a
	 * ref="http://code.google.com/p/google-rfc-2445/">http
	 * ://code.google.com/p/google-rfc-2445/</a>
	 */
		...
		while (iter.hasNext() && num++ < 10000)
		{
			...
			if (d instanceof com.google.ical.values.DateTimeValue)
			{
				...
				try
				{
					...
					/*
					 * if ( this.untilDate != null && newDateTime.isAfter (
					 * this.untilDate ) ) { break; }
					 */
			...
			} else
			{
				try
				{
					...
					// the "UNTIL=" setting, so we will enforce it here.
					/*
					 * if ( this.untilDate != null && newDate.isAfter (
					 * this.untilDate ) ) { break; }
					 */
