...
public class RapplaWidgetProvider extends AppWidgetProvider
{
	...
	public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds)
	{
		...
		try
		{
			...
			text = nextEvent.getTitle() + '\n' + startTime.get(Calendar.DAY_OF_MONTH) + '/' + (startTime.get(Calendar.MONTH) % 12 + 1) + '/'
					+ startTime.get(Calendar.YEAR) + '\n' +

					startTime.get(Calendar.HOUR_OF_DAY) + ':' + startTime.get(Calendar.MINUTE) + '-' + endTime.get(Calendar.HOUR_OF_DAY) + ':'
